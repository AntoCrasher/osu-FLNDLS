...
def color_text(text, color=''):
    ...
    if color == 'ERROR':
        ...
        return f'\033[38;2;255;255;255mz{text}\033[0m'
...
def FLNDLS(songs_path, beatmapset_path, beatmap_name, p_index, p_max, harder=False):
    ...
    if beatmap_name.count('[') > 0:
        new_beatmap_name = f'{beatmap_name.split('[')[0]}[FLNDLS{buff} - {beatmap_name.split('[')[1]}'
    ...
    if changes == 0:
        print(progress + color_text(f'Already FLNDLS{buff}: {beatmap_name}', 'WARNING'))
        return

    with open(new_beatmap_path, 'w', encoding='utf-8') as file:
        file.write(new_data)
    print(progress + color_text(f'Done: {beatmap_name}', 'ACCEPT'))
...
def main():
    ...
    for beatmapset_name in beatmapsets:
        ...
        for beatmap_name in beatmaps:
            ...
            FLNDLS(songs_path, beatmapset_path, beatmap_name, index, len(beatmapsets), False)
...
def single():
    ...
    for beatmap_name in beatmaps:
        ...
        FLNDLS(songs_path, beatmapset_path, beatmap_name, 1, 1, False)
        FLNDLS(songs_path, beatmapset_path, beatmap_name, 1, 1, True)
